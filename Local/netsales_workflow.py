...
def analysis_of_data(natural_language_report):
    ...
    prompt = f"""
        ### **Context:**
        The following data is summarization of the changes in Netsales between two periods.
...
def summary(analysis):
    ...
    prompt = f"""
        ### **Context:**
        The following data is from an analyis of the changes in Netsales between two periods.
        In the analysis there is also the magnitude as well if it is considered a minor/major increase/decrease. 
...
def compilation(dataset, product_area_list, business_area):
    with open("Netsales.txt", "a", encoding="utf-8") as file:  # Open in append mode
        for product_area in product_area_list:
            ...
            print(f"Response for {product_area} saved to response.txt!")
